...
export default async function GatilhoPage({
  ...
  const [
    ...
    { data: relatosRaw, error: errRelatos  },
  ] = await Promise.all([
  ...
  if (errPeriodos) console.error('[gatilho] periodos_disponiveis:', errPeriodos.message)
  if (errRelatos)  console.error('[gatilho] gatilho_relato:', errRelatos.message)
