...
// Vercel Hobby permite no máximo 60s; 55 deixa margem de segurança
...
export async function POST(req: NextRequest) {
  ...
  const rl = checkRateLimit(`importar:${user.id}`, 60, 300_000)
  if (!rl.allowed) {
  ...
  if (batchNum === 1) {
    ...
    if (impExist && impExist.length > 0) {
      ...
      if (cddExistente !== novoCDD) {
        ...
        const { error: delDevErr } = await supabase.from('devolucoes').delete().in('importacao_id', ids)
        if (delDevErr) return NextResponse.json({ error: `Erro na limpeza: ${delDevErr.message}` }, { status: 500 })
        const { error: delImpErr } = await supabase.from('importacoes').delete().in('id', ids)
        if (delImpErr) return NextResponse.json({ error: `Erro na limpeza: ${delImpErr.message}` }, { status: 500 })
        limpeza = 'total'
      } else {
        ...
        const { error: delDevErr } = await supabase.from('devolucoes').delete().eq('cdd', novoCDD).in('periodo', periodosParaLimpar)
        if (delDevErr) return NextResponse.json({ error: `Erro na limpeza: ${delDevErr.message}` }, { status: 500 })
        ...
        const { data: comDev, error: comDevErr } = await supabase
          .from('devolucoes')
          .select('importacao_id')
          .in('importacao_id', importIds)
        if (comDevErr) console.error('[importar] erro ao verificar devolucoes:', comDevErr.message)
        ...
        if (orphans.length) {
          const { error: delOrphanErr } = await supabase.from('importacoes').delete().in('id', orphans)
          if (delOrphanErr) console.error('[importar] erro ao remover importacoes órfãs:', delOrphanErr.message)
        }
        limpeza = 'periodo'
      }
    }
